    ...
    >
      ...
      <div class="flex items-end justify-between mt-5">
        ...
        <span
          ...
          :class="getTrendClass(metricasMensuales.clientes?.porcentaje)"
        >
          <svg
            :class="['fill-current', (metricasMensuales.clientes?.porcentaje || 0) < 0 ? 'rotate-180' : '']"
            width="12"
            height="12"
            viewBox="0 0 12 12"
            fill="none"
          ...
          {{ Math.abs(metricasMensuales.clientes?.porcentaje || 0).toFixed(1) }}%
        </span>
      </div>
    </div>

    <!-- Card 2: Préstamos este mes -->
    <div
      class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6"
    >
      <div
      ...
      <div class="flex items-end justify-between mt-5">
        ...
        <span
          class="flex items-center gap-1 rounded-full py-0.5 pl-2 pr-2.5 text-sm font-medium"
          :class="getTrendClass(metricasMensuales.prestamos?.porcentaje)"
        >
          <svg
            :class="['fill-current', (metricasMensuales.prestamos?.porcentaje || 0) < 0 ? 'rotate-180' : '']"
            width="12"
            height="12"
            viewBox="0 0 12 12"
            fill="none"
          ...
          {{ Math.abs(metricasMensuales.prestamos?.porcentaje || 0).toFixed(1) }}%
        </span>
      </div>
    </div>
  </div>
...
const { metricasMensuales } = storeToRefs(dashboardStore)

const getTrendClass = (percentage) => {
  if (percentage === undefined || percentage === null) return 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400'
  return percentage >= 0
